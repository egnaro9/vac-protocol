...
audit.
\item A refusal-site deletion mutation operator, with an explicit kill predicate, an
explicit exclusion set, and a validity gate that makes the measurement trustworthy. The
operator is a narrow instance of statement deletion; what is new here is the site selection
and the subject, not the deletion (Section~\ref{sec:related-unclaimed}).
...
how the intervention is defined.
...
experience reading someone's spec''.\footnote{An earlier draft of this paper rendered that
...
\citep{ami2021muse}. G\"orz et al.\ extend mutation analysis to grading fuzzers
...
The direction matters and I state it rather than leave a reviewer to find it. muSE, G\"orz
et al.\ and Chen and Furia all mutate the checker's \emph{input}, seeding defects into the
artifact under analysis and asking whether the checker detects them. Chen and Furia
...
paper points the mutation tool \emph{at} the gate.
...
grade it is muSE's and Chen and Furia's. The green-baseline precondition in
...
What remains unclaimed by prior work, as far as I can establish, is the combination of three
things: the \emph{site selection} (refusal statements that are the sole point of rejection
for one class of bad input), the \emph{subject} (an evidence verifier's own acceptance
logic, whose verdict is published), and the \emph{detector} (a unit suite plus a committed
corpus of tampered bundles, so a mutant can be caught by a test or by a fixture ceasing to
be refused, and the two can be scored separately). The third of these is what produced the
paper's least comfortable number: scored on its own, the corpus written to prove the
verifier can refuse catches 10 of 112 (Section~\ref{sec:rq1}).
...
Five things, and nothing broader.
...
\item A refusal-site deletion mutation operator and a liveness workflow for applying it,
including the validity gate that makes the measurement trustworthy. The operator family is
prior art (Section~\ref{sec:related-unclaimed}); the site selection, the subject, and the
two-detector kill predicate are what is offered.
...
systems broadly, or that this methodology transfers.
