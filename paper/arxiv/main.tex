...
That first pull request filed five distinct forgery paths, plus path traversal on an
untested Python window. His second pull request found something worse in kind: no \texttt{encoding=} on
...
\texttt{RESULTS.md} byte-identity check caught mojibake in an auditor's output mid-review.
The instrument worked on the reviewer. That exchange happened in correspondence rather than
in the pull request record, so unlike every other claim in this section it rests on no
public artifact; it is a first-person report, it is marked as one, and it is left
unattributed for that reason.
